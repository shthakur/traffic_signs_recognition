...
class FC(nn.Module):
    ...
    def forward(self, input):
        # input = input.view(-1, self.input_nbr)
...
class LocalizationNetwork(nn.Module):
    ...
    def __init__(self, conv_params, input_size, input_channels=3):
        ...
        self.fc = FC(conv_output_size, conv_params[2])
        self.classifier = Classifier(conv_params[2], self.nbr_params)
        self.classifier.lin.weight.data.zero_()
        self.classifier.lin.bias.data.copy_(self.init_bias)

    def forward(self, input):
        input = nn.functional.max_pool2d(input, 2)
...
class GeneralNetwork(nn.Module):
    def __init__(self, opt):
        ...
        conv_output_size, _ = get_convs_output_size([self.conv1, self.conv2])
        self.fc = FC(conv_output_size, conv_params[2])
        self.classifier = Classifier(conv_params[2], NUM_CLASSES)
        self.locnet_1 = None
...
class IDSIANetwork(GeneralNetwork):
    def __init__(self, opt):
        ...
        conv_output_size, _ = get_convs_output_size([self.conv1,
                                                     self.conv2,
                                                     self.conv3])
        self.fc = FC(conv_output_size, conv_params[3])
        self.classifier = Classifier(conv_params[3], NUM_CLASSES)
    ...
    def forward(self, input):
        ...
        return self.classifier(self.fc(input))
